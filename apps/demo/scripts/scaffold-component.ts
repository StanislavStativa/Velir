...
const componentDestPath = `${componentRootPath}`;
const prettierOpts = {
  parser: 'babel-ts',
...
(async () => {
  ...
  const componentOutputPath = scaffoldFile(
    componentDestPath,
    await format(generateComponentSrc(componentName), prettierOpts), // format the output
    filename
  );

  const storyOutputPath = scaffoldFile(
    componentDestPath,
    await format(generateComponentUiStory(componentName), prettierOpts),
    `${kebabCase(componentName)}.stories.tsx`
  );

  const mockDataOutputPath = scaffoldFile(
    componentDestPath,
    await format(generateComponentMockData(componentName), prettierOpts),
    `${kebabCase(componentName)}.mock.ts`
  );

  const propsOutputPath = scaffoldFile(
    componentDestPath,
    await format(generateComponentUiProps(componentName), prettierOpts),
    `${kebabCase(componentName)}.props.tsx`
  );

  const utilsOutputPath = scaffoldFile(
    componentDestPath,
    await format(generateComponentUtils(componentName), prettierOpts),
    `${kebabCase(componentName)}.util.ts`
  );
  ...
  function scaffoldFile(
    ...
    const outputDir = path.join(rootPath, componentPath, kebabCase(parentDirectory));
