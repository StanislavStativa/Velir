...
import { Link, useSitecoreContext, Text } from '@sitecore-jss/sitecore-jss-nextjs';
...
export const Default: React.FC<ArticleListingProps> = ({
  ...
  const articles: TransformedArticle[] = React.useMemo(() => {
    ...
    return featuredContent.map((article) => ({
      ...
      author:
        `${article.fields.taxAuthor?.fields?.personFirstName?.value || ''} ${article.fields.taxAuthor?.fields?.personLastName?.value || ''}`.trim(),
          ...
          <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
            ...
            <div className="@md:grid-cols-2 mb-[28px] grid gap-8">
              {featuredArticles.map((article, index) => (
                <div key={index} className="@md:mb-0 group mb-6">
                  {isPageEditing ? (
                  ...
                  <div className="@md:p-8">
                    {isPageEditing ? (
                      ...
                      <Link field={{ value: { href: article.link } }} className="block">
                        <h3 className="font-heading text-card-foreground text-3xl font-medium leading-[1.30] -tracking-[0.9px] decoration-1 underline-offset-4 group-hover:underline group-focus:underline">
                          {article.title}
                        </h3>
                      </Link>
                    )}
            ...
            <div className="@sm:grid-cols-2 @lg:grid-cols-3 grid gap-8">
              {regularArticles.map((article, index) => (
                <div
                  key={index}
                  className="@md:p-8 rounded-default hover:bg-tertiary-hover focus:ring-accent group flex h-full flex-col p-4 transition-colors focus:outline-none focus:ring-2"
                >
                  <div>
                    {isPageEditing ? (
                      ...
                      <Link field={{ value: { href: article.link } }} className="block">
                        <h3 className="font-heading text-card-foreground text-2xl font-medium leading-normal tracking-tighter decoration-1 underline-offset-4 group-hover:underline group-focus:underline">
                          {article.title}
                        </h3>
                      </Link>
                    )}
