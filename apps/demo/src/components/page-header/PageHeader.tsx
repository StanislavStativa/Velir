...
const pageHeaderComponentClasses = cva(
  '@container my-10 flex flex-col md:grid md:gap-8 md:items-center px-4 @xl:px-8 [.px-4_&]:px-0 [.px-4_&]:@xl:px-0 [.@xl\:px-8_&]:xl:px-0',
  {
    variants: {
      colorScheme: {
        default: '',
        ...
        <div className="@md:grid-cols-2 @lg:gap-12 mx-auto grid w-full grid-cols-1 gap-8">
          ...
          <div className="flex flex-col justify-between">
            <AnimatedSection reducedMotion={prefersReducedMotion} isPageEditing={isPageEditing}>
              <Text
                tag="h1"
                className="@md:text-6xl @lg:text-8xl font-heading text-pretty text-5xl font-normal leading-[0.93] tracking-tighter"
                field={title}
              />
            </AnimatedSection>
            {children?.results && (
